...
class SXDataStructurePreviewer extends SXBasePropertiesPanelComponent {
  ...
  listenerUpdateParameterComments = (event) => {
    ...
    if (parameter.hasComments()) {
      this.comments = (
        <div>
          <TreeView defaultItems={parameter.comments} nestedKey="replies">
            {(item) => {
              <TreeView.item>
                <TreeView.ItemStack>
                  <Icon symbol="reply" spritemap={this.spritemap} />
                  {item.name}
                </TreeView.ItemStack>
              </TreeView.item>;
            }}
    ...
    this.addCommentDlgBody = (
      <div>
        <Text size={6} style={{ marginBottom: '1.5rem' }}>
        ...
        {this.commentParam.hasValue() && (
          <pre style={{ marginBottom: '1.5rem' }}>{JSON.stringify(this.commentParam.getValue(), null, 4)}</pre>
        )}
              ...
              {this.jumpTo && (
                <Toolbar.Section>
                  <ClayInput.Group>
                    ...
                    <ClayInput.GroupItem append expand="true">
                      <Autocomplete
                        aria-labelledby={Util.translate('find-parameter')}
                        id=""
                        defaultItems={this.dataStructure.getJumpToItems().map((item) => item.name)}
                        messages={{
            ...
            {this.inputStatus && (
              <Toolbar.Item>
                <Toolbar.Section>
                  <ClayInput.Group>
                    <ClayInput.GroupItem shrink>
                      <ClayInput.GroupText style={{ backgroundColor: '#f3ded7', borderRadius: '1.0rem' }}>
                        {this.dataStructure.valuedFieldsCount +
                          '/' +
                          this.dataStructure.totalFieldsCount +
                          '(' +
                          ((this.dataStructure.valuedFieldsCount / this.dataStructure.totalFieldsCount) * 100).toFixed(
                            1
                          ) +
                          '%)'}
                      </ClayInput.GroupText>
