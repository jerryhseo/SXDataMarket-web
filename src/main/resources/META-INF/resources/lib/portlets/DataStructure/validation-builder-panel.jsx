...
class SXDSBuilderValidationPanel extends SXBasePropertiesPanelComponent {
  constructor(props) {
    ...
    const baseValueString = createParameter({
    ...
    const baseValueNumeric = createParameter({
      namespace: this.namespace,
      formId: this.componentId,
    ...
    const baseBoundary = createParameter({
      namespace: this.namespace,
      formId: this.componentId,
    ...
    const baseMessage = createParameter({
      namespace: this.namespace,
      formId: this.componentId,
      paramType: ParamType.STRING,
      properties: {
    ...
    const baseErrorLevel = createParameter({
      namespace: this.namespace,
      formId: this.componentId,
  ...
  listenerFieldValueChanged = (event) => {
    ...
    if (!(targetPortlet == this.namespace && targetFormId == this.componentId)) {
      console.log('SXDSBuilderValidationPanel SX_FIELD_VALUE_CHANGED: ', event.dataPacket, this.workingParam);
      return;
    }

    console.log('SXDSBuilderValidationPanel SX_FIELD_VALUE_CHANGED: ', this.workingParam, parameter, cellIndex);
    ...
    switch (parameter.paramCode) {
      ...
      case 'minLengthValue': {
        ...
        if (Util.isNotEmpty(maxLength)) {
          if (minLength > maxLength) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('min-length-must-be-smaller-than-max-length'));
      ...
      case 'maxLengthValue': {
        ...
        if (Util.isNotEmpty(minLength)) {
          if (maxLength < minLength) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('max-length-must-be-larger-than-min-length'));
      ...
      case 'minValue': {
        ...
        if (Util.isNotEmpty(normalMin)) {
          if (min > normalMin) {
            this.minProps.value.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-normal-min'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-normal-min'));
        ...
        if (Util.isNotEmpty(normalMax)) {
          if (min > normalMax) {
            this.minProps.value.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-normal-max'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-normal-max'));
        ...
        if (Util.isNotEmpty(max)) {
          if (min > max) {
            this.minProps.value.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-max'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('min-value-must-be-smaller-than-max'));
      ...
      case 'maxValue': {
        ...
        if (Util.isNotEmpty(normalMin)) {
          if (max < normalMin) {
            this.maxProps.value.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-normal-min'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-normal-min'));
        ...
        if (Util.isNotEmpty(normalMax)) {
          if (max < normalMax) {
            this.maxProps.value.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-normal-max'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-normal-max'));
        ...
        if (Util.isNotEmpty(min)) {
          if (max < min) {
            this.maxProps.value.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-min'));
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('max-value-must-be-larger-than-min'));
      ...
      case 'normalMinValue': {
        ...
        if (Util.isNotEmpty(min)) {
          if (normalMin < min) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('normal-min-value-must-be-larger-than-min'));
        ...
        if (Util.isNotEmpty(max)) {
          if (normalMin > max) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('normal-min-value-must-be-smaller-than-max'));
      ...
      case 'normalMaxValue': {
        ...
        if (Util.isNotEmpty(min)) {
          if (normalMax < min) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('normal-max-value-must-be-larger-than-min'));
        ...
        if (Util.isNotEmpty(max)) {
          if (normalMax > max) {
            ...
            this.dataStructure.setError(ErrorClass.ERROR, Util.translate('normal-max-value-must-be-smaller-than-max'));
  ...
  enableSection(sectionId, enable) {
    if (enable) {
      ...
      this.workingParam.setDirty(false);
    }
  ...
  handleToggle(sectionId, enable) {
    //console.log('handleToggle: ', sectionId, enable);
    ...
    return (
      <div className="autofit-row">
        ...
        <span className="autofit-col">
          <ClayToggle
            onToggle={(e) => this.handleToggle(sectionId, e)}
  ...
  renderSectionBody(sectionId) {
    switch (sectionId) {
      ...
      case 'min': {
        return (
          <>
            <div className="autofit-row autofit-padded" style={{ marginBottom: '0.5rem' }}>
          ...
          <>
            <div className="border sx-validation-section">
              ...
              {this.checkSectionEnabled(ValidationKeys.MIN_LENGTH) && (
                <div style={{ marginLeft: '2rem' }}>{this.renderSectionBody(ValidationKeys.MIN_LENGTH)}</div>
              )}
            </div>
            <div className="border sx-validation-section">
              ...
              {this.checkSectionEnabled(ValidationKeys.MAX_LENGTH) && (
                <div style={{ marginLeft: '2rem' }}>{this.renderSectionBody(ValidationKeys.MAX_LENGTH)}</div>
              )}
            </div>
          </>
        )}
          ...
          <>
            ...
            <div className="border sx-validation-section">
              ...
              {this.checkSectionEnabled(ValidationKeys.NORMAL_MIN) && (
                <div style={{ marginLeft: '2rem' }}>{this.renderSectionBody(ValidationKeys.NORMAL_MIN)}</div>
              )}
            </div>
            <div className="border sx-validation-section">
              ...
              {this.checkSectionEnabled(ValidationKeys.NORMAL_MAX) && (
                <div style={{ marginLeft: '2rem' }}>{this.renderSectionBody(ValidationKeys.NORMAL_MAX)}</div>
              )}
            </div>
          </>
        )}
