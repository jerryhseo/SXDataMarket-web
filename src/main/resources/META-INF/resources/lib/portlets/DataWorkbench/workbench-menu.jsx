        ...
        <NavigationBar style={{ ...this.style }}>
          {this.menuItems.map((menuItem) => {
            if (Util.isNotEmpty(menuItem.children)) {
              return (
                <NavigationBar.Item key={menuItem.id} style={{ marginRight: '10px' }}>
                  <DropDown
                    active={this.state.active === menuItem.id}
                    alignmentByViewport={true}
                    closeOnClick={true}
                    trigger={
                      <Link style={{ fontWeight: '550', fontSize: '1.0rem' }} spritemap={this.spritemap}>
                        {menuItem.label}
                      </Link>
                    }
                    onActiveChange={(val) => {
              ...
              return (
                <NavigationBar.Item key={menuItem.id} active={this.state.active === menuItem.id}>
                  <Link
