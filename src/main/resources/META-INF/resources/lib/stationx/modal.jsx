...
export class SXModalUtil {
  ...
  static errorDlgHeader(spritemap) {
    return (
      <div style={{ color: 'red' }}>
        <Icon symbol="exclamation-full" spritemap={spritemap} style={{ marginRight: '1rem' }} />
  ...
  static warningDlgHeader(spritemap) {
    return (
      <div style={{ color: '#a0870aff' }}>
        <Icon symbol="warning" spritemap={spritemap} style={{ marginRight: '1rem' }} />
  ...
  static successDlgHeader(spritemap) {
    return (
      <div style={{ color: 'green' }}>
        <Icon symbol="check-circle" spritemap={spritemap} style={{ marginRight: '1rem' }} />
...
export const SXModalDialog = ({
  ...
  return (
    <Modal
      ...
      spritemap={spritemap}
    >
      <Modal.Header withTitle={withTitle} style={{ backgroundColor: '#dcf09c' }}>
        <h3>{header}</h3>
      </Modal.Header>
...
export const SXNoticeDialog = ({ size, spritemap, header, body }) => {
  ...
  return (
    <ClayModal observer={observer} size={size} spritemap={spritemap} status="info">
  ...
  return {
    payload: {
      ...
      footer: buttons.map((button, index) => {
        return (
          <ClayButton key={index} onClick={button.onClick}>
            {button.label}
          </ClayButton>
        );
      }),
...
export const SXErrorModal = ({ imageURL }) => {
  ...
  return (
    <Modal observer={observer} disableAutoClose={true} center>
