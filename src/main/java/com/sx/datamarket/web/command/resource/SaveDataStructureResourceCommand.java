...
public class SaveDataStructureResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( strDataStructure.isEmpty () ) {
			result.put ( "error", SXLocalizationUtil.translate ( resourceRequest, "nothing-to-save" ) );
		...
		if ( dataTypeId > 0 ) {
			...
			if ( Validator.isNotNull ( dataType ) ) {
				...
				result.put (
							...
							SXLocalizationUtil.translate (
										resourceRequest,
										"datastructure-for-the-datatype-is-saved",
										dataTypeId
							)
				);
			} else {
				result.put (
							...
							SXLocalizationUtil.translate (
										resourceRequest,
										"cannot-find-datatype-to-save-datastructure",
										dataTypeId
							)
				);
			...
			result.put ( "message", SXLocalizationUtil.translate ( resourceRequest, "datastructure-is-saved" ) );
		} else {
			// Check duplicated
			...
			result.put (
						...
						SXLocalizationUtil.translate (
									resourceRequest,
									"datastructure-is-added",
									new long[] {dataStructureId}
						)
			);
		}
		...
		Path referencePath = Paths.get (
					stationXDataDir + "/" + companyId + "/" + themeDisplay.getScopeGroupId () + "/referenceFiles"
		);

		Path structurePath =
					Validator.isNotNull ( dataType ) ? referencePath
								.resolve ( dataType.getDataTypeCode () + "/" + dataType.getDataTypeVersion () )
								: referencePath
											.resolve (
														jsonDataStructure.getString ( "dataStructureCode" ) + "/"
																	+ jsonDataStructure
																				.getString ( "dataStructureVersion" )
											);
		...
		for ( int i = 0; i < parameters.length (); i++ ) {
			...
			if ( hasReferenceFile ) {
				if ( SXUtil.contains ( fileFields, paramCode ) ) {
					...
					SXFileUtil.emptyFolder ( versionFolderPath, true );
					...
					JSONArray errorFiles =
								SXFileUtil.saveUploadFieldFiles ( uploadRequest, paramCode, versionFolderPath );
				...
				SXFileUtil.deleteFolder ( codeFolderPath );
			}
		}
