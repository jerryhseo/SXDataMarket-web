...
@Component(
			immediate = true,
			property = {"javax.portlet.name=" + WebPortletKey.SXDataSetViewerPortlet,
...
public class ViewDataSetResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( dataSetId == 0 ) {
			result.put ( "error", SXLocalizationUtil.translate ( resourceRequest, "dataset-id-should-be-specified" ) );
		...
		if ( dataCollectionId > 0 && dataSetId > 0 ) {
			...
			List<SetTypeLink> setTypeLinkList = _setTypeLinkLocalService
						.getSetTypeLinkListByCollectionSet ( dataSet.getGroupId (), dataCollectionId, dataSetId );
