	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( dataSetId == 0 && _dataSetLocalService.checkDuplicated ( dataSetCode, dataSetVersion ) ) {
			result.put (
						"error",
						SXLocalizationUtil
									.translate ( resourceRequest, "dataset-is-duplicated", dataSetCode, dataSetVersion )
			);
		...
		for ( int order = 0; order < strAryAssociatedDataTypes.length; order++ ) {
			...
			if ( Validator.isNull ( setTypeLink ) ) {
				setTypeLink = _setTypeLinkLocalService
							.addSetTypeLink ( dataCollectionId, dataSetId, dataTypeId, order, setTypeLinkSC );

				System.out.println (
							"SetTypeLink Added: " + setTypeLink.getPrimaryKey () + ", " + setTypeLink.getGroupId ()
										+ ", " + setTypeLink.getDataCollectionId () + ", " + setTypeLink.getDataSetId ()
		...
		result.put ( "message", SXLocalizationUtil.translate ( resourceRequest, "dataset-saved-as", dataSetId ) );
