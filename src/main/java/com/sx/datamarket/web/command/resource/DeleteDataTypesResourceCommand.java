...
public class DeleteDataTypesResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( errorObjects.length () > 0 ) {
			result.put (
						"message",
						LanguageUtil.get ( resourceRequest.getLocale (), "cannot-delete-some-of-datatypes" )
			);
			result.put ( "failedDataTypes", errorObjects );
		}

		result.put ( "deletedDataTypes", strDataTypeIds );
