...
public class SearchDataTypesResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		JSONArray dataTypeList = null;
		if ( dataCollectionId > 0 && dataSetId > 0 ) {
			dataTypeList = _dataTypeLocalService.getLinkedDataTypeListByCollectionSet (
						groupId,
						dataCollectionId,
						dataSetId,
			...
			dataTypeList = _dataTypeLocalService
						.getLinkedDataTypeListByCollection ( groupId, dataCollectionId, themeDisplay.getLocale () );
		} else if ( dataSetId > 0 ) {
			dataTypeList = _dataTypeLocalService
						.getLinkedDataTypeListByDataSet ( groupId, dataSetId, themeDisplay.getLocale () );
		} else {
			dataTypeList = _dataTypeLocalService.getLinkedDataTypeList ( groupId, themeDisplay.getLocale () );
		}
