...
public class DeleteStructuredDataResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( strDataIds.isEmpty () ) {
			result.put (
						"error",
						SXLocalizationUtil.translate ( resourceRequest, "data-id-should-be-specified-to-be-deleted" )
			);
		...
		Path sxDataFolderPath = Paths.get ( stationXDataDir + "/" + companyId + "/" + themeDisplay.getScopeGroupId () );
		...
		if ( Validator.isNotNull ( dataType ) ) {
			sxDataFolderPath =
						sxDataFolderPath.resolve ( dataType.getDataTypeCode () + "/" + dataType.getDataTypeVersion () );
		}
		...
		for ( int i = 0; i < strAryDataIds.length; i++ ) {
			...
			_structuredDataLocalService.removeStructuredData ( Long.parseLong ( strAryDataIds[i] ), sxDataFolderPath );
		}

		result.put (
					"message",
					SXLocalizationUtil.translate ( resourceRequest, "data-were-deleted-successfully", strAryDataIds )
		);
