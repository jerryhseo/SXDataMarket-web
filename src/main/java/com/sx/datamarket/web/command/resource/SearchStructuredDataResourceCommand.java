...
public class SearchStructuredDataResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( dataCollectionId > 0 && dataSetId > 0 && dataTypeId > 0 ) {
			dataList = _structuredDataLocalService.getStructuredDatasByCollectionSetType (
						groupId,
						dataCollectionId,
						dataSetId,
						dataTypeId,
			...
			dataList = _structuredDataLocalService.getStructuredDatasByCollectionSet (
						groupId,
						dataCollectionId,
						dataSetId,
						WorkflowConstants.STATUS_ANY
			...
			dataList = _structuredDataLocalService.getStructuredDatasByDataCollectionId (
						groupId,
						dataCollectionId,
						WorkflowConstants.STATUS_ANY
			);
		} else if ( dataSetId > 0 && dataTypeId > 0 ) {
			dataList = _structuredDataLocalService
						.getStructuredDatasBySetType ( groupId, dataSetId, dataTypeId, WorkflowConstants.STATUS_ANY );
		} else if ( dataSetId > 0 ) {
			dataList = _structuredDataLocalService
						.getStructuredDatasByDataSetId ( groupId, dataSetId, WorkflowConstants.STATUS_ANY );
		} else if ( dataTypeId > 0 ) {
			dataList = _structuredDataLocalService
						.getStructuredDatasByDataTypeId ( groupId, dataTypeId, WorkflowConstants.STATUS_ANY );
		} else {
			dataList = _structuredDataLocalService
						.getStructuredDatasByGroupId ( groupId, WorkflowConstants.STATUS_ANY );
		}
