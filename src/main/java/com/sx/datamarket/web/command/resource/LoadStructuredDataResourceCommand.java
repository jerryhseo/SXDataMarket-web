...
public class LoadStructuredDataResourceCommand extends BaseMVCResourceCommand {
	...
	protected void doServeResource ( ResourceRequest resourceRequest, ResourceResponse resourceResponse )
		...
		if ( structuredDataId == 0 ) {
			result.put (
						"error",
						SXLocalizationUtil.translate ( resourceRequest, "structured-data-id-should-be-specified" )
			);
		...
		if ( ! ( dataCollectionId == structuredData.getDataCollectionId ()
			...
			result.put (
						...
						SXLocalizationUtil.translate (
									resourceRequest,
									"structured-data-info-mismatch-to-load",
									dataCollectionId,
									dataSetId,
									dataTypeId
						)
		...
		if ( _dataTypeLocalService.hasDataStructure ( dataTypeId ) ) {
			...
			if ( Validator.isNull ( dataStructure ) ) {
				result.put (
							...
							SXLocalizationUtil.translate (
										resourceRequest,
										"datatype-structure-doesnt-exist-for-the-data",
										new long[] {structuredDataId}
							)
				);
				SXPortletURLUtil.responeAjax ( resourceResponse, result );
			...
			result.put (
						...
						SXLocalizationUtil.translate (
									resourceRequest,
									"datatype-doesnt-exist-for-the-data",
									new long[] {structuredDataId}
						)
			);
			SXPortletURLUtil.responeAjax ( resourceResponse, result );
