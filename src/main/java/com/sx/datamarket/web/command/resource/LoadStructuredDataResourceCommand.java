...
public class LoadStructuredDataResourceCommand extends BaseMVCResourceCommand{
	...
	protected void doServeResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse)
		...
		if( structuredDataId == 0 ) {
			result.put("error", SXUtil.translate(resourceRequest, "structured-data-id-should-be-specified") );
		...
		if( Validator.isNull(structuredData) ) {
			result.put("error", SXUtil.translate(resourceRequest, "cannot-find-structured-data", new long[] {structuredDataId}));
			SXPortletURLUtil.responeAjax(resourceResponse, result);
		...
		if( _dataTypeLocalService.hasDataStructure(dataTypeId) ) {
			...
			if( Validator.isNull(dataStructure) ) {
				result.put("error", SXUtil.translate(resourceRequest, "datatype-structure-doesnt-exist-for-the-data", new long[] {structuredDataId}));
				SXPortletURLUtil.responeAjax(resourceResponse, result);
			...
			result.put("error", SXUtil.translate(resourceRequest, "datatype-doesnt-exist-for-the-data", new long[] {structuredDataId}));
			SXPortletURLUtil.responeAjax(resourceResponse, result);
